"""Async base classes and mixins for IFPA API client architecture.

This module provides the foundational async base classes and mixins that eliminate
code duplication across async resource clients, context managers, and query builders.

The architecture is built on three key abstractions:

1. AsyncBaseResourceContext: Base for async resource-specific context managers
2. AsyncBaseResourceClient: Base for async resource client classes
...
while maintaining type safety and minimizing code duplication in the async context.
"""
...
# Type variable for AsyncBaseResourceContext resource_id type
T = TypeVar("T")


class AsyncBaseResourceContext(Generic[T]):
    """Base class for async resource-specific context managers.

    This base class provides common initialization for async context managers
    that handle operations on a specific resource instance (e.g., a specific player,
    director, or tournament).

    Context managers are returned by calling async resource clients with a resource ID,
    ...
    Attributes:
        _http: The async HTTP client instance for making API requests
    ...
    def __init__(self, http: _AsyncHttpClient, resource_id: T, validate_requests: bool) -> None:
        """Initialize an async resource context.

        Args:
            http: The async HTTP client instance for making API requests
...
class AsyncBaseResourceClient:
    """Base class for async resource client classes.

    This base class provides common initialization for async resource clients
    that manage both collection-level operations and individual resource access.

    Async resource clients serve as the entry point for API operations on a specific
    ...
    Example:
        ...
        class AsyncPlayerClient(AsyncBaseResourceClient):
            ...
            def search(self, name: str = "") -> AsyncPlayerQueryBuilder:
                return AsyncPlayerQueryBuilder(self._http, self._validate_requests)
        ```
    """

    def __init__(self, http: _AsyncHttpClient, validate_requests: bool) -> None:
        """Initialize the async resource client.
...
class LocationFiltersMixin:
    ...
    This mixin is used by both sync and async query builders since it only manipulates
    parameters without making network calls.

    Example:
        ```python
        class AsyncPlayerQueryBuilder(AsyncQueryBuilder, LocationFiltersMixin):
            # Inherits country(), state(), city() methods

        # Usage
        async with AsyncIfpaClient() as client:
            query = client.player.search().country("US").state("WA").city("Seattle")
    ...
    def country(self, country_code: str) -> Self:
        ...
        Example:
            ...
            async with AsyncIfpaClient() as client:
                results = await client.player.search().country("US").get()

            # Reusable base query
            async with AsyncIfpaClient() as client:
                us_query = client.player.search().country("US")
                wa_players = await us_query.state("WA").get()
                or_players = await us_query.state("OR").get()
            ```
        """
    ...
    def state(self, stateprov: str) -> Self:
        ...
        Example:
            ...
            # US state
            async with AsyncIfpaClient() as client:
                results = await client.player.search().state("WA").get()

            # Canadian province
            async with AsyncIfpaClient() as client:
                results = await client.player.search().country("CA").state("BC").get()
            ```
        """
        clone = self._clone()  # type: ignore[attr-defined]
...
class PaginationMixin:
    ...
    Example:
        ```python
        class AsyncPlayerQueryBuilder(AsyncQueryBuilder, PaginationMixin):
            # Inherits limit(), offset() methods

        # Usage - paginate through results
        async with AsyncIfpaClient() as client:
            page_size = 25
            page_1 = await client.player.search("Smith").limit(page_size).get()
            page_2 = await client.player.search("Smith").offset(25).limit(page_size).get()
            page_3 = await client.player.search("Smith").offset(50).limit(page_size).get()
        ```
    """
    ...
    def limit(self, count: int) -> Self:
        ...
        Example:
            ...
            async with AsyncIfpaClient() as client:
                rankings = await client.rankings.wppr(count=10)  # Returns 10 results

            # Player search - count is ignored, returns 50
            async with AsyncIfpaClient() as client:
                players = await client.player.search("Smith").limit(10).get()  # Returns 50

            # Use offset() for pagination on search endpoints
            async with AsyncIfpaClient() as client:
                page1 = await client.player.search("Smith").offset(0).get()   # First 50
                page2 = await client.player.search("Smith").offset(50).get()  # Next 50
            ```
        """
        clone = self._clone()  # type: ignore[attr-defined]
    ...
    def offset(self, start_position: int) -> Self:
        ...
        Example:
            ...
            async with AsyncIfpaClient() as client:
                results = await client.player.search("Smith").offset(25).limit(25).get()

            # Skip first 100 results
            async with AsyncIfpaClient() as client:
                results = await client.player.search().country("US").offset(100).get()
            ```
        """
        clone = self._clone()  # type: ignore[attr-defined]
